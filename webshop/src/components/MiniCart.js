...
class Basket extends Component {
  render() {
    ...
    if (cartItems === undefined) {
      return <div className="alert alert-info">"Basket is empty"</div>;
    }
        ...
        {cartItems.length === 0 ? (
          ...
          <div className="itemsInMiniCart">
            {cartItems.map((item) => (
              <div id={item.id}>
                ...
                <GiveBackPrice
                  ...
                  symbolUSD={item.prices[0].currency.symbol}
                  priceGBP={item.prices[1].amount}
                  symbolGBP={item.prices[1].currency.symbol}
                  priceRUB={item.prices[4].amount}
                  symbolRUB={item.prices[4].currency.symbol}
                  priceAUD={item.prices[2].amount}
                  symbolAUD={item.prices[2].currency.symbol}
                  priceJPY={item.prices[3].amount}
                  symbolJPY={item.prices[3].currency.symbol}
                />
                <div>
                  <Attributes
            ...
            <b>
              Sum:{" "}
              {cartItems.reduce(
                (a, c) => a + c.prices[currencyvariable].amount * c.count,
                0
              )}{" "}
              {currencysymbolvariables}{" "}
            </b>
